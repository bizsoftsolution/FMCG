...
namespace FMCG.BLL
{
    public class Supplier : INotifyPropertyChanged
    {
        ...
        public static ObservableCollection<Supplier> toList
        {
            get
            {
                try
                {
                    if (_toList == null)
                    {
                        _toList = new ObservableCollection<Supplier>();
                        var l1 = FMCGHubClient.FMCGHub.Invoke<List<Supplier>>("Supplier_List").Result;
                        _toList = new ObservableCollection<Supplier>(l1);
                    }
                }
                catch (Exception ex)
                {
        ...
        public bool Delete(bool isServerCall = false)
        {
            ...
            {
                toList.Remove(d);
                if (isServerCall == false) FMCGHubClient.FMCGHub.Invoke<int>("Supplier_Delete", this.Id);
                return true;
            }
